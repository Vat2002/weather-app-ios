...
class WeatherMapViewModel: ObservableObject {
    ...
    func getCoordinatesForCity(cityName: String) async throws {
        ...
        //                let locations: [Location] = loadLocationsFromJSONFile(cityName: city) ?? []
        //
    ...
    func loadData(lat: Double, lon: Double) async throws -> WeatherDataModel {
        ...
        if let url = URL(string: "https://api.openweathermap.org/data/3.0/onecall?lat=\(lat)&lon=\(lon)&appid=\(Constants.secretKey)&units=metric") {
